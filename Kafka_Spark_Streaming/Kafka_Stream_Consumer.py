...
def create_consumer_group(topic_name, number_of_consumers):
    ...
    for i in range(0, number_of_consumers):
        consumer_group.append(consumer_thread(i, topic_name))
    return consumer_group


def create_consumer(topic_name):
    parsed_topic_name = 'topic_' + topic_name
    return KafkaConsumer(parsed_topic_name, auto_offset_reset='earliest',
                         bootstrap_servers=['localhost:9092'], api_version=(0, 10), consumer_timeout_ms=1000)


class consumer_thread(threading.Thread):
    def __init__(self, threadID, topic_name):
        threading.Thread.__init__(self)
        self.threadID = threadID
        self.topic = topic_name
        self.name = topic_name + "consumer" + str(threadID)
        self.consumer = create_consumer(topic_name)
